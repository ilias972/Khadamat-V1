...
exports.pauseBetweenRequests = pauseBetweenRequests;
exports.extractLinks = extractLinks;
exports.crawlSite = crawlSite;
exports.inspectPage = inspectPage;
...
async function pauseBetweenRequests(config) {
...
async function extractLinks(page, config) {
    ...
    for (const href of anchors) {
        ...
        if (!normalized)
            continue;
        if (shouldExclude(normalized, config))
            continue;
        if (config.sameOriginOnly && new URL(normalized).origin !== new URL(config.baseUrl).origin)
            continue;
        unique.add(normalized);
    }
    return Array.from(unique);
}
async function crawlSite(page, config, collector) {
...
async function inspectPage(page, url, config, collector) {
    const issues = [];
    const consoleErrors = [];
    const onConsole = (msg) => {
    ...
    try {
        ...
        const missingAlt = await page.$$eval('img', (imgs) => imgs.filter((img) => !img.getAttribute('alt')).length);
        if (missingAlt > 0) {
            issues.push({
                severity: 'medium',
        ...
        const formsInfo = await page.$$eval('form', (forms) => forms.map((form) => {
            ...
            return { action: form.action, required, inputs };
        }));
        if (formsInfo.length) {
    ...
    if (config.screenshotOnIssue && issues.length) {
        const folder = path_1.default.join(process.cwd(), 'test-results', 'screenshots');
        fs_1.default.mkdirSync(folder, { recursive: true });
        const fileSafeName = url.replace(/https?:\/\//, '').replace(/[^a-z0-9.-]/gi, '_');
        const filePath = path_1.default.join(folder, `${fileSafeName}.png`);
        if (!page.isClosed()) {
